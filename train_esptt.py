...
def train(epoch, config, model, training_data, optimizer, logger, visualizer=None):
    ...
    for step, (inputs, inputs_length, targets, targets_length) in enumerate(training_data):
        ...
        del loss, inputs, targets, inputs_length, targets_length
...
def main():
    ...
    parser.add_argument('-config', type=str, default='config/espnet_aishell.yaml')
    ...
    model = TransformerTransducer(config.model)
